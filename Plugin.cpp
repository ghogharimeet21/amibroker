...
#include<chrono>
//#include <fmt/core.h>
#include"temp_files/WebSocketHandler.h"
				...
				if( poFile->ReadString( oLine ) )
				{
					if( oLine == "OK" )
					{
						while( poFile->ReadString( oLine ) )
						{
							...
							int hour = 12;
							int minute = safe_atoi(strtok(line, ":."));
...
PLUGINAPI int GetQuotesEx( LPCTSTR pszTicker, int nPeriodicity, int nLastValid, int nSize, struct Quotation *pQuotes, GQEContext *pContext  )
{
	...
	else
	{
		...
		//TRACE("strart time %d %s %d %d \n", nStartTime, pszTicker, nLastValid , nPeriodicity);
